...
Test Reliability & Stability (Critical Automation Principles):
- Hardcoded waits (cy.wait ≥ 5000ms) → Anchor: cy.wait(number) call where number ≥ 5000. Default: evidence=4, confidence=0.8.
- Brittle selectors (nth-child, complex CSS, auto-generated classes) → Anchor: selector string. Default: evidence=4, confidence=0.8.
- Tests without proper isolation (shared beforeEach state, no cleanup) → Anchor: shared variable usage or missing cleanup hooks. Default: evidence=4, confidence=0.8.
- Missing Page Object patterns causing code duplication → Anchor: repeated selector/action patterns across tests. Default: evidence=4, confidence=0.8.
- Unbounded operations or infinite loops in tests → Anchor: loop without exit condition or timeout. Default: evidence=4, confidence=0.8.

Test Organization & Maintainability (Code Quality):
- Focused/skipped tests (it.only, it.skip, xit) committed to branches → Anchor: test modifier. Default: evidence=3, confidence=0.6.
- Tests disabling browser security without proper guards → Anchor: security config. Default: evidence=3, confidence=0.6.
- Tests without descriptive names → Anchor: it() or describe() with unclear names. Default: evidence=3, confidence=0.7.
- README.md files in subdirectories (auto-generated by Cursor) → Anchor: new README.md file in subdirectory. Default: evidence=4, confidence=0.8.
