...
- Hardcoded waits (cy.wait ≥ 5000ms or long wait) → Anchor: cy.wait(number) call where number ≥ 5000. Fix: use cy.intercept() or conditional waits with cy.should(). Default: evidence=4, confidence=0.8.
...
- Missing Page Object patterns causing code duplication → Anchor: repeated selectors/actions. Fix: extract to page objects or custom commands. Default: evidence=4, confidence=0.8.
- Non-deterministic selectors (brittle and complex XPath, auto-generated classes) → Anchor: brittle selector. Fix: use data-testid or accessibility queries. Default: evidence=4, confidence=0.8.
...
- Focused/skipped tests (it.only, it.skip, xit) committed to main branches → Anchor: test modifier. Fix: remove modifier before merge. Default: evidence=3, confidence=0.6.
- Tests disabling browser security without proper guards → Anchor: security config. Fix: guard with environment checks or remove. Default: evidence=3, confidence=0.6.
- Tests without descriptive names or proper organization → Anchor: unclear test/describe name. Fix: use descriptive test names and proper grouping. Default: evidence=3, confidence=0.8.
