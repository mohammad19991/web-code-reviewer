...
class LoggingService {
  ...
  logReviewDetails(
    ...
    core.info(
      `  - PR Number: ${process.env.GITHUB_EVENT_NAME === 'pull_request' ? process.env.GITHUB_EVENT_NUMBER : 'Not available'}`
    );
  ...
  logFinalDecision(shouldBlockMerge, llmResponse) {
      ...
      if (extractedData.chunksProcessed > 0) {
        ...
        if (shouldBlockMerge) {
          const criticalIssues = extractedData.issues.filter(
            i => i.severity_proposed === 'critical'
          );
          const highConfidenceCritical = criticalIssues.filter(i => i.confidence >= 0.6);

          core.setFailed(
            `🚨 MERGE BLOCKED: LLM review found ${criticalIssues.length} critical issues (${highConfidenceCritical.length} with high confidence ≥ 0.6) across ${extractedData.chunksProcessed} chunks`
          );
          ...
          if (highConfidenceCritical.length > 0) {
            ...
            highConfidenceCritical.forEach(issue => {
              core.info(
                `   - ${issue.originalId}: ${issue.category} (Chunk ${issue.chunk}, score: ${issue.severity_score?.toFixed(1) || 'N/A'}, ${Math.round(issue.confidence * 100)}% confidence)`
              );
              core.info(`     File: ${issue.file}, Lines: ${issue.lines.join('-')}`);
              if (issue.risk_factors) {
                core.info(
                  `     Risk Factors: I:${issue.risk_factors.impact} E:${issue.risk_factors.exploitability} L:${issue.risk_factors.likelihood} B:${issue.risk_factors.blast_radius} Ev:${issue.risk_factors.evidence_strength}`
                );
              }
          ...
          const suggestions = extractedData.issues.filter(
            i => i.severity_proposed === 'suggestion'
          );
          core.info(
            `✅ MERGE APPROVED: No critical issues found across ${extractedData.chunksProcessed} chunks. ${suggestions.length} suggestions available for consideration.`
          );
        ...
        core.info(
          `📊 Review Summary: ${extractedData.totalCriticalCount} critical, ${extractedData.totalSuggestionCount} suggestions across ${extractedData.chunksProcessed} chunks`
        );
    ...
    if (shouldBlockMerge) {
      core.setFailed(
        '🚨 MERGE BLOCKED: LLM review found critical issues that must be addressed before merging.'
      );
